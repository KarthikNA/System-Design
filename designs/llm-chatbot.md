...
### 7.1 RAG vs. Fine-Tuning
RAG and fine-tuning are both ways to specialise a model for a domain, but they serve different needs. RAG keeps knowledge external and updatable - adding new documents to the Vector DB immediately improves responses without touching the model. Fine-tuning bakes knowledge into the model's weights, which produces faster inference (no retrieval step) and better stylistic alignment, but requires expensive retraining cycles every time the knowledge changes. For most production chatbots, RAG is the default choice; fine-tuning is reserved for cases where response style, tone, or domain vocabulary needs deep alignment that prompting alone cannot achieve. A hybrid approach - RAG on top of a lightly fine-tuned model - is increasingly common for high-stakes deployments.

### 7.2 Semantic Cache Threshold - Accuracy vs. Hit Rate
The similarity threshold for the semantic cache is a direct dial between response accuracy and cache efficiency. A high threshold (≥ 0.95) ensures only genuinely equivalent queries are served from cache, but results in a low hit rate that limits cost savings. A low threshold (< 0.85) maximises cache reuse but risks returning a response that does not truly answer the user's question, degrading trust. There is no universal correct value - it must be tuned empirically per deployment based on the query distribution and how much semantic drift the application can tolerate. It is also worth noting that cached responses do not reflect any knowledge updates made to the Vector DB after the cache entry was written, so TTL management is critical.

### 7.3 Guardrails - Safety vs. Latency
Every guardrail check adds latency. Running both input and output checks synchronously adds two blocking round-trips to the critical path. For applications where safety is non-negotiable (regulated industries, public-facing products), this overhead is justified and unavoidable. For lower-risk internal tools, operators may choose to run output guardrails asynchronously - logging and flagging violations for review rather than blocking the response in real time. The risk is that a harmful response reaches the user before the flag is caught. Another option is batching guardrail calls or using lighter classification models that sacrifice some accuracy for speed. The right balance depends entirely on the risk profile of the deployment.

### 7.4 Model Routing - Quality vs. Cost vs. Latency
Routing decisions sit at the intersection of three competing forces. Sending every request to the most capable model maximises quality but is prohibitively expensive at scale. Routing everything to the cheapest model reduces cost but degrades responses for complex queries. Latency adds a third dimension - a fast mid-tier model may produce a better user experience than a slower frontier model even if the latter is technically more capable. Getting routing right requires a well-calibrated Model Selection Model and continuous monitoring of output quality per tier. Misrouting - especially under-routing complex queries to weak models - is often invisible in metrics until user satisfaction drops.

### 7.5 Self-Hosted vs. External LLMs
Self-hosted models give full control over data, cost structure, and latency, but come with significant operational burden - GPU infrastructure, model versioning, scaling, and on-call requirements. External providers offload all of that in exchange for per-token cost, third-party data exposure, and dependence on provider availability and pricing. For most startups, external providers are the right default. For enterprises handling sensitive data or operating at very high volumes where per-token costs compound significantly, self-hosted models become increasingly attractive. A practical hybrid - external models for general traffic, self-hosted for sensitive or high-volume workloads - is the most common production pattern.

### 7.6 Context Window Management - Truncation vs. Summarisation
When conversation history exceeds the model's context window, something has to give. Truncation (dropping the oldest messages) is simple and cheap but loses context that may still be relevant - a user who referenced something three turns ago will feel like the assistant has forgotten them. Summarisation preserves continuity by compressing old turns into a brief summary, but introduces its own latency (a separate LLM call to generate the summary) and risk (the summary may omit or distort details). Summarisation is the better experience; truncation is the simpler engineering choice. For most deployments, a hybrid works well - truncation during normal conversation, with summarisation triggered only when the session exceeds a defined age or turn count.

### 7.7 Streaming vs. Buffered Responses
Streaming tokens to the client as they are generated dramatically reduces perceived latency - the user sees a response begin forming within hundreds of milliseconds rather than waiting several seconds for the full output. However, streaming adds meaningful infrastructure complexity: long-lived connections must be maintained across the API Gateway and Chat Service, output Guardrails must operate on a partial stream rather than a complete response, and error handling mid-stream requires special care. For conversational UX, streaming is almost always worth the complexity. For API integrations where the caller needs the full response before processing (e.g. downstream pipelines, structured output parsing), buffered responses are simpler and safer.

### 7.8 Vector Index - Recall vs. Speed
The choice between a FLAT (brute-force) and HNSW (approximate nearest-neighbour) index in the Vector DB and Redis cache is a recall-vs-speed trade-off. FLAT guarantees finding the true nearest neighbours but scans all vectors linearly - it becomes impractically slow beyond ~100K entries. HNSW finds approximate nearest neighbours in O(log n) time with very high recall (typically 95–99%), making it the production standard for large-scale deployments. The small recall gap means HNSW may occasionally miss a relevant cached response or RAG chunk that FLAT would have found - acceptable in most cases, but worth monitoring if retrieval quality degrades as the index grows.

### 7.9 Embedding Model - Quality vs. Cost vs. Latency
All vector search quality - both in the semantic cache and in RAG - depends on the embedding model. Larger, higher-dimensional models (e.g. OpenAI `text-embedding-3-large` at 3072 dims) produce richer representations and better retrieval quality but are slower and more expensive to call, and their larger vectors consume more storage and memory in the Vector DB. Smaller models (e.g. `all-MiniLM-L6-v2` at 384 dims) are fast, cheap, and compact but may miss subtle semantic similarities. Critically, the same model must be used at both ingestion and query time - switching models requires re-embedding the entire knowledge base, which is a costly migration. Embedding model choice should be treated as a long-term infrastructure commitment.
